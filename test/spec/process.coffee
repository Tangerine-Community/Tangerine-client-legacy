(() ->
  ...
  tests = (dbName)->
    ...
    describe 'Should process bulk uploads', ()->
      this.timeout(10000);
...
#      it('Should return the expected assessment - process', (done)->
...
#      )
      ...
      it('Should process the bulk upload', (done)->
        this.timeout(20000)
#        Note that am currently sending only to local pouch; need to send to master CouchDB ultimately.
#        This would all go into a method in helps.js Utils class called @loadOfflineBulkDocs: (callback) ->
        $.ajax
          dataType: "json"
          url: "uploads/new/backup.txt"
          error: (res) ->
            console.log("Error fetching file: " + res)
          success: (res) ->
            docs = {"docs":res.rows.map((el)->el.doc)}
#               Look at Utils.uploadCompressed - do we need to LZ compress it before sending to bulkdocs?
#              compressedData = LZString.compressToBase64(JSON.stringify(docs))
            Tangerine.db.bulkDocs docs, (error, doc) ->
              ...
              if error
                console.log("Error bulkdocs: " + error)
              done()
      )

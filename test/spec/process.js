(function() {
  ...
  tests = function(dbName) {
    ...
    return describe('Should process bulk uploads', function() {
      this.timeout(10000);
      dbs = [];
      ...
      return it('Should process the bulk upload', function(done) {
        this.timeout(20000);
        return $.ajax({
          dataType: "json",
          url: "uploads/new/backup.txt",
          error: function(res) {
            return console.log("Error fetching file: " + res);
          },
          success: function(res) {
            var docs;
            docs = {
              "docs": res.rows.map(function(el) {
                return el.doc;
              })
            };
            return Tangerine.db.bulkDocs(docs, function(error, doc) {

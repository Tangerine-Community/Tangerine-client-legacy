...
SurveyEditView = (function(_super) {
  ...
  SurveyEditView.prototype.addQuestion = function() {
    ...
    newAttributes = $.extend(Tangerine.config.questionTemplate, {
      ...
      name: this.$el.find('#question_name').val().replace(/\s/g, "_").replace(/[^a-zA-Z0-9_]/g, "")
    });
  ...
  SurveyEditView.prototype.save = function() {
    var emptyOptions, i, notSaved, question, requiresGrid, _len, _ref;
    this.model.set({
    ...
    if (emptyOptions.length !== 0) {
      alert("Warning\n\n" + (emptyOptions.length > 1 ? "Questions" : "Question") + " " + (emptyOptions.join(' ,')) + " has no options.");
    }
    if (requiresGrid.length !== 0) {
      return alert("Warning\n\n" + (requiresGrid.length > 1 ? "Questions" : "Question") + " " + (requiresGrid.join(' ,')) + " requires a grid to be linked to this test.");
    }
  };
